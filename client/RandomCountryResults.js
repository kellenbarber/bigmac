...
const RandomCountryResults = props => {
    ...
    return (
        <div className={"randomCountryResults"}>
            ...
            <p className={"numberOfBigMacs"}>You could buy {Math.floor(numberOfBigMacs)} Big Macs in {bigMacData.Country} with {amount}!</p>
            <p className={"localValue"}>Your {amount} is worth about {currencyValue.toFixed(2)} in {bigMacData.Country}.</p>
        </div>
    );
};
